...
#include <AggregateFunctions/AggregateFunctionWindowFunnelTime.h>
...
createAggregateFunctionWindowFunnelTime(const std::string & name, const DataTypes & arguments, const Array & params, const Settings *)
{
    if (params.empty())
    ...
    AggregateFunctionPtr res(createWithUnsignedIntegerType<AggregateFunctionWindowFunnelTime, Data>(*arguments[0], arguments, params));
    WhichDataType which(arguments.front().get());
    if (res)
        return res;
    else if (which.isDate())
        return std::make_shared<AggregateFunctionWindowFunnelTime<DataTypeDate::FieldType, Data<DataTypeDate::FieldType>>>(arguments, params);
    else if (which.isDateTime())
        return std::make_shared<AggregateFunctionWindowFunnelTime<DataTypeDateTime::FieldType, Data<DataTypeDateTime::FieldType>>>(arguments, params);
...
void registerAggregateFunctionWindowFunnelTime(AggregateFunctionFactory & factory)
{
    factory.registerFunction("windowFunnelTime", createAggregateFunctionWindowFunnelTime<AggregateFunctionWindowFunnelTimeData>);
}
