    ...
    def run(self):
        while not self.queue.empty():
            ...
            try:
                ...
                continue
            if in_changelog:
                desc = generate_description(api_pr)
                if desc is not None:
...
def generate_description(item: PullRequest) -> Optional[Description]:
    ...
    if from_ref is None:
        ...
        try:
            ...
            if FROM_REF != alternative_tag:
                raise Exception(
                    "Unable to get unified parent tag, define it manually, "
                    f"get {FROM_REF} and {alternative_tag}"
                )
        except CalledProcessError:
            pass
    else:
